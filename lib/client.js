  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      var onCaptureClick = function (event) {
        ...
        // Ignore clicks on floating control buttons (whale or close)
        if (targetEl.closest && targetEl.closest('.dsh-overlay-whale-btn, .dsh-overlay-close-btn')) return;
      ...
      var onOverlayBgClick = function (event) {
        if (!document.documentElement.hasAttribute("data-dsh-overlay")) return;
        var frame = document.querySelector('[class*="_frame"]');
        if (frame && !frame.hasAttribute("data-sidebar-collapsed")) return; // Don't minimize if sidebar is open
        var target = event.target;
        if (!target || !(target instanceof Element)) return;
        if (target.closest('.dsh-overlay-whale-btn, .dsh-overlay-close-btn, [data-composer-card], [class*="_card"], [class*="_bubble"], [class*="_callRow"], [data-chat-flow-kind], button, input, textarea, a, select')) {
          return;
        }
        try {
      ...
      document.addEventListener("click", onOverlayBgClick, true);
      ...
      ctx.effect(function () {
        return function () {
          ...
          document.removeEventListener("click", onOverlayBgClick, true);
        };
      });
    }
