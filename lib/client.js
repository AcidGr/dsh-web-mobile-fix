window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  scrollbar-width: none !important;",
      "}",
      "/* Hero empty state: hide fish, headline, workspace chip */",
    ...
    function apply(ctx) {
      ...
      if (isOverlayMode) {
        ...
        // Clear session immediately upon overlay launch
        clearSession();

        // Briefly keep session cleared during initial async backend bootstrap
      ...
      var onCaptureClick = function (event) {
        ...
        // Ignore clicks on the whale button
        if (targetEl.closest && targetEl.closest('.dsh-overlay-whale-btn')) return;
      ...
      var onOverlayBgClick = function (event) {
        ...
        if (frame && !frame.hasAttribute("data-sidebar-collapsed")) return; // Don't close activity if sidebar is open
        var target = event.target;
        if (!target || !(target instanceof Element)) return;
        if (target.closest('.dsh-overlay-whale-btn, [data-composer-card], [class*="_card"], [class*="_bubble"], [class*="_callRow"], [data-chat-flow-kind], button, input, textarea, a, select')) {
          return;
        }
        try {
          if (window.DSHOverlayBridge && typeof window.DSHOverlayBridge.close === "function") {
