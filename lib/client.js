window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // Auto-collapse full-screen sidebar on mobile/overlay when a session or new session is selected
      var onSidebarItemClick = function (event) {
        ...
        var sessionRow = target.closest && target.closest(
          '[class*="_sessionRow"], [class*="_searchResultRow"], button[class*="_newSession"], button[aria-label*="新会话"], button[aria-label*="新建会话"]'
        );
        if (sessionRow) {
          setTimeout(function () {
            try {
