window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      "  box-sizing: border-box !important;",
      "}",
      "html[data-dsh-overlay=\"true\"] [class*=\"_frame\"]:not([data-sidebar-collapsed])::after {",
      "  content: \"\" !important;",
      "  position: fixed !important;",
      "  inset: 0 !important;",
      ...
      "  background-color: transparent !important;",
      "}",
      "html[data-dsh-overlay=\"true\"] [class*=\"_header\"] {",
      "  display: none !important;",
      "}",
      ...
      "}",
      "/* Hide all tool calls and turn process pills for pure chat dialogue */",
    ...
    function apply(ctx) {
      ...
      if (isOverlayMode) {
        ...
        // Reset to clean Form A (Hero empty state) on overlay startup
        try {
          localStorage.removeItem("dsh.sessions.current");
          sessionStorage.removeItem("dsh.sessions.current");
        } catch (e) {}

        var ensureBlankSession = function () {
          try {
            var sessions = ctx.get ? ctx.get("sessions") : void 0;
            var workspaces = ctx.get ? ctx.get("workspaces") : void 0;
            if (!sessions || !workspaces) return false;
            ...
            if (!sessionSnap || !workspaceSnap) return false;
            if (sessionSnap.phase !== "ready" || workspaceSnap.phase !== "ready") return false;

            var current = sessionSnap.current;
            var currentSummary = current ? sessionSnap.byId[current] : void 0;
            // If already in a blank session, we're golden!
            if (currentSummary && currentSummary.blank) return true;

            // Find recent or default workspace
            var targetWorkspace = void 0;
            var latestTime = -Infinity;
            var items = workspaceSnap.items || [];
            for (var i = 0; i < items.length; i++) {
              var ws = items[i];
              var wsTime = -Infinity;
              var sids = ws.sessionIds || [];
              for (var j = 0; j < sids.length; j++) {
                var s = sessionSnap.byId[sids[j]];
                if (s && s.updatedAt) wsTime = Math.max(wsTime, s.updatedAt);
              }
              if (wsTime === -Infinity && ws.createdAt) wsTime = Date.parse(ws.createdAt);
              if (targetWorkspace === void 0 || wsTime > latestTime) {
                targetWorkspace = ws;
                latestTime = wsTime;
              }
            }

            if (!targetWorkspace && items.length > 0) targetWorkspace = items[0];
            if (!targetWorkspace) {
              if (typeof sessions.clear === "function") sessions.clear();
              return true;
            }

            // Look for an existing blank session in this workspace
            ...
            // Otherwise create a new blank session in this workspace
        ...
        // Ensure clean blank session in active workspace on overlay launch
        ensureBlankSession();

        // Keep blank session active during initial async backend bootstrap
        var initTimer = setInterval(function () {
          ensureBlankSession();
        }, 150);

        // Cancel timer as soon as user touches or interacts with the screen
        var stopInitTimer = function () {
          if (initTimer) {
            clearInterval(initTimer);
            initTimer = null;
          }
        };
        setTimeout(stopInitTimer, 1500);
        document.addEventListener("pointerdown", stopInitTimer, { once: true, capture: true });
        document.addEventListener("keydown", stopInitTimer, { once: true, capture: true });
      }
      ...
      /* Unified Overlay Click & Dismissal State Machine:
         ...
         Level 3: Any open Menu, Dropdown, Listbox, Popover, or Modal Dialog:
                  ...
                  (DSH Menus dismiss on pointerdown, so we track popup existence on pointerdown to prevent timing races).
      ...
      var hadActivePopupOnPointerDown = false;
      var onPointerDownTrack = function (event) {
        ...
        if (activePopup && target && !activePopup.contains(target)) {
          hadActivePopupOnPointerDown = true;
        } else {
          hadActivePopupOnPointerDown = false;
        }
      };
      document.addEventListener("pointerdown", onPointerDownTrack, true);
      document.addEventListener("touchstart", onPointerDownTrack, true);
      ...
      var onOverlayClick = function (event) {
        ...
        if (target.closest && target.closest('.dsh-overlay-whale-btn, .dsh-overlay-close-btn')) {
          return;
        }
        ...
        // Level 3: Check if a popup was dismissed during this tap sequence (pointerdown/touchstart)
        if (hadActivePopupOnPointerDown) {
          hadActivePopupOnPointerDown = false;
          return; // Popup was dismissed by this tap! DO NOT minimize the app!
        }

        // Direct check: Is any Menu, Dropdown, Dialog, Listbox, or Popover still open in the DOM?
        var activePopup = document.querySelector(
          '[role="menu"], [role="listbox"], [role="dialog"], [role="alertdialog"], ' +
          '[data-dockkit-tab-menu], [data-floating-ui-portal] > *, [class*="Menu_menu"]'
        );
      ...
      ctx.effect(function () {
        return function () {
          ...
          document.removeEventListener("touchstart", onPointerDownTrack, true);
